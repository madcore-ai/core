...
pushd /opt/madcore/kubernetes/cluster/ssl/
  ...
  cp tocken.csv /opt/kubernetes/ssl/
  cp worker-kubeconfig.yaml /opt/kubernetes/ssl/
  cat worker-openssl.cnf_template | sed -e "s/\${KUB_MASTER_IP}/$ip}/" | sed -e "s/\$${local_node_ip}/$local_ip}/" > /opt/kubernetes/ssl/openssl.cnf
popd
