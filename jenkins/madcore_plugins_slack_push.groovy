job('madcore.plugin.slack.push') {
    ...
    parameters {
        ...
        stringParam('SLACK_PAYLOAD', '{"channel": "tests", "text": "Hi from docker container!"}', '')
    }
    steps {
...
echo "SLACK_PAYLOAD: '\$SLACK_PAYLOAD'"
    ...
    python /opt/madcore/bin/deploy_container.py "\$REPO_URL" "\$APP_NAME" "slack-token=\$SLACK_TOKEN" "madcore_slack -a post_message -p '\$SLACK_PAYLOAD'"
"""
        shell(command)
    }
}
